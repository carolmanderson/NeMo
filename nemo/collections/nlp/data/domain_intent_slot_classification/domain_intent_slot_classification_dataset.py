...
__all__ = ['DomainIntentSlotClassificationDataset', 'DomainIntentSlotInferenceDataset']
...
def get_features(
    ...
    for i, query in enumerate(queries):
        ...
        if extra_cls_token:
            subtokens = [tokenizer.cls_token, '[CLS2]']
            loss_mask = [1 - ignore_start_end, 1 - ignore_start_end]
            subtokens_mask = [0, 0]
            if with_label:
                slots = [pad_label, pad_label]
    ...
    for i, subtokens in enumerate(all_subtokens):
        if len(subtokens) > max_seq_length:
            if extra_cls_token:
                subtokens = [tokenizer.cls_token, '[CLS2]'] + subtokens[-max_seq_length + 2 :]
                all_input_mask[i] = [1, 1] + all_input_mask[i][-max_seq_length + 2 :]
                all_loss_mask[i] = [1 - ignore_start_end, 1 - ignore_start_end] + all_loss_mask[i][-max_seq_length + 2 :]
                all_subtokens_mask[i] = [0, 0] + all_subtokens_mask[i][-max_seq_length + 2 :]
                if with_label:
                    all_slots[i] = [pad_label, pad_label] + all_slots[i][-max_seq_length + 2 :]
            else:
...
class DomainIntentSlotClassificationDataset(Dataset):
    """
    ...
    Args:
        ...
        extra_cls_token: if True, add [CLS2] after [CLS]; this will be used for domain classification
    """
    ...
    def output_types(self) -> Optional[Dict[str, NeuralType]]:
        ...
        return {
            ...
            'domain_labels' : NeuralType(('B'), LabelsType()),
    ...
    def __init__(
        ...
        raw_slots, queries, raw_intents, raw_domains = [], [], [], []
        for slot_line, input_line in dataset:
            raw_slots.append([int(slot) for slot in slot_line.strip().split()])
            parts = input_line.strip().split()
            raw_intents.append(int(parts[-2]))
            raw_domains.append(int(parts[-1]))
            query = ' '.join(parts[:-2])
            if do_lower_case:
                query = query.lower()
        ...
        features = get_features(
            ...
            extra_cls_token=extra_cls_token
        )
    ...
    def __getitem__(self, idx):
        return (
            ...
            self.all_domains[idx],
            self.all_intents[idx],
            np.array(self.all_slots[idx]),
        )


class DomainIntentSlotInferenceDataset(Dataset):
    """
    Creates dataset to use for the task of joint intent
    and slot classification with pretrained model.
    ...
    def __init__(self, queries, max_seq_length, tokenizer, do_lower_case, extra_cls_token=False):
        if do_lower_case:
...
if __name__ == "__main__":
    ds = DomainIntentSlotClassificationDataset("/Users/carola/Documents/data/domain_merging/combined_domains/merged_with_domain/train.tsv",
                                         "/Users/carola/Documents/data/domain_merging/combined_domains/merged_with_domain/train_slots.tsv",
