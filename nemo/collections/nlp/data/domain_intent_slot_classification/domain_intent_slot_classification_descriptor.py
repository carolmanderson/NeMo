...
class DomainIntentSlotDataDesc:
    ...
    def __init__(
        ...
        if not if_exist(data_dir, ['dict.domains.csv', 'dict.intents.csv', 'dict.slots.csv']):
        ...
        self.domains_label_ids = DomainIntentSlotDataDesc.label2idx(self.domain_dict_file)
        self.num_domains = len(self.domains_label_ids)
        self.intents_label_ids = DomainIntentSlotDataDesc.label2idx(self.intent_dict_file)
        self.num_intents = len(self.intents_label_ids)
        self.slots_label_ids = DomainIntentSlotDataDesc.label2idx(self.slot_dict_file)
        ...
        for mode in modes:
            ...
            logging.info(f'Three most popular domains in {mode} mode:')
            total_domains, domain_label_freq, max_id = get_label_stats(
                raw_domains, infold + f'/{mode}_domain_stats.tsv'
            )
            ...
            if mode == 'train':
                domain_weights_dict = get_freq_weights(domain_label_freq)
                logging.info(f'Intent Weights: {domain_weights_dict}')
        ...
        self.domain_weights = fill_class_weights(domain_weights_dict, self.num_domains - 1)
...
if __name__ == "__main__":
    ...
    config_file = os.path.join(HOME_DIR, "configs/joint_domain_intent_slot/domain_intent_slot_classification_config.yaml")
    config = OmegaConf.load(config_file)
    print(config)

    data_dir = "/Users/carola/Documents/data/domain_merging/combined_domains/merged_with_domain"
    config.model.data_dir = data_dir

    DomainIntentSlotDataDesc(data_dir,
        modes=["train", "dev"])